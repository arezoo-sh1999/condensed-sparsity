...
class RigLConstFanScheduler(RigLScheduler):
    ...
    def _get_neurons_to_ablate(
        ...
        if not (self.dynamic_ablation and self.min_salient_weights_per_neuron != 0):
            if self.static_ablation:
                return list(self.static_ablated_filters)
            return []

        dense_fan_in = math.prod(weight.shape[1:])
        n_ones_max = int(mask.numel() * (1 - sparsity))
        min_neurons = max(1, int(n_ones_max // dense_fan_in))
        ...
        # حداقل یک نورون باقی بماند
        if len(neurons_to_ablate) >= num_neurons:
            neurons_to_ablate = neurons_to_ablate[: num_neurons - 1]
    
        # حداکثر 30٪ نورون‌ها حذف شوند
        max_ablate = int(0.3 * num_neurons)
        if len(neurons_to_ablate) > max_ablate:
            neurons_to_ablate = neurons_to_ablate[-max_ablate:]
    
        # تضمین حداقل نورون فعال
        if (num_neurons - len(neurons_to_ablate)) < min_neurons:
            allowed_ablate = max(0, num_neurons - min_neurons)
            neurons_to_ablate = neurons_to_ablate[:allowed_ablate]
